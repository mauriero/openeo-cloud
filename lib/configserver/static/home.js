...
const initialSocLabel = document.getElementById("initialSocPct");
...
let currentLimYRelPos = 0.0;   // place slider at centre vertically for Initial SOC
...
// Schedule is saved after no alterations to schedule for 10 seconds.
const scheduleSaveAuto = 10000;
// Initial SOC is saved after 10 seconds of inactivity.
const initialSocSaveAuto = 10000;
let initialSocSaveTimer = null;

function scheduleInitialSocSave() {
    if (initialSocSaveTimer) {
        clearTimeout(initialSocSaveTimer);
    }
    initialSocSaveTimer = setTimeout(() => {
        saveInitialSoc();
    }, initialSocSaveAuto);
}

function saveInitialSoc() {
    // Persist only the initial SOC; openeo.py will detect change and reset session energy
    fetch('/setconfig', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ "chargeroptions" : { "initial_soc_pct": currentLimVal } })
    }).then(() => { makeToast('Saved Initial SOC'); })
    .catch(() => {});
}
...
function updateCurrentLimText() {
    ...
    initialSocLabel.innerText = tempLimVal.toString();
    currentLimText.innerHTML = ""; // not used for text anymore
}
...
function snapCurrent() {
	...
	console.log('New initial SOC for mode ', currentMode, ' is ', currentLimVal);
	
	currentLimPos = (currentLimVal - minCurrentLim) / (maxCurrentLim - minCurrentLim);
	updateCurrentLimText();
    console.log('Schedule save: initial soc (10s debounce)');
    scheduleInitialSocSave();
	drawUI();
	updateStatus();
...
function saveSchedule(report) {
	...
	.then(data => {
        if (report) { makeToast('Saved new schedule'); }
	})
	.catch(error => {
...
function updateStatus(once) {
	...
	if (once || ((updateTick % updateFreq) == 0)) {
		...
		.then(function(stat) {
            ...
            // Flags display
            try {
                const lm = stat['loadmanagement'] || {};
                const solarEnabled = !!lm['solar_enable'];
                const topupEnabled = !!lm['solar_topup_enable'];
                flagSolar.style.display = solarEnabled ? '' : 'none';
                flagTopup.style.display = (solarEnabled && topupEnabled) ? '' : 'none';
                document.getElementById('flagsDisplay').style.visibility = 'visible';
            } catch (e) {}

            // Current SOC calculation if initial SOC > 0
            try {
                const initSoc = parseInt((stat['chargeroptions']||{})['initial_soc_pct'] || 0);
                const delivered = parseFloat(stat['eo_session_energy_kwh'] || 0);
                const cap = parseFloat((stat['loadmanagement']||{})['ev_battery_capacity_kwh'] || 40);
                if (initSoc > 0 && cap > 0) {
